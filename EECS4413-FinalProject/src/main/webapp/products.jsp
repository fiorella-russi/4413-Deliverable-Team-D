...
<body>
	...
	<div class="side-panel">
		...
		<button onclick="compareProducts()" id="compare-button">Compare
		Selected</button>
		</div>
	...
	<div id="itemContainer">
        ...
        %>
		<div class="item card" data-color="<%= color %>"
			data-make="<%= make %>" data-model="<%= model %>"
			data-price="<%= price %>" data-km="<%= km %>"
			data-accident="<%= accident %>" data-type="<%= type %>">
			<div class="item-content">
				<a href="productDetails?param1=<%= item.get("id") %>"> <img
					src="<%= item.get("image1") %>" alt="Item Image">	</a>  <br><%= color +" "+make +" "+model %><br>
					 <%= dollarFormat.format(price) %>
					<br>KM: <%=numberFormat.format(km) %> <br>Accident History: <%= accident %></br>
<br>
					<div class="compare-checkbox">
						<label> <input type="checkbox" name="compareVehicle"
							value="<%= item.get("id") %>"> Add to comparison
						</label>
					</div>
